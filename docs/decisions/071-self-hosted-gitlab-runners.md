...
The manager's egress to the API server is a `CiliumNetworkPolicy` using `toEntities: [kube-apiserver]`, not a plain `NetworkPolicy` `ipBlock` rule. Under Cilium's kube-proxy replacement, traffic to the API server — whether dialed via the in-cluster Service ClusterIP (`10.96.0.1:443`) or the control-plane node address directly (`10.0.0.226:6443`) — is tagged with the reserved `kube-apiserver` identity, and `ipBlock`/CIDR rules never match reserved identities, only arbitrary world CIDRs. Everything else in both namespaces (DNS, internet egress) uses plain `NetworkPolicy`, since `ipBlock` matches those destinations correctly.
...
- The manager's API-server rule tracks Cilium's `kube-apiserver` reserved identity rather than a pinned address, so a control-plane rebuild on another address (single control plane per ADR-016) doesn't require touching this policy — Cilium re-resolves the identity from the `kubernetes` Service endpoints on its own.
