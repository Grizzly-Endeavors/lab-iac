...
- **Nine accounts migrated with their credentials preserved byte-for-byte,** so no consumer secret in 1Password changed and nothing downstream needed resyncing. The copy ran through the admin API, account by account, reading the old records out of the OpenBao mount with the same AppRole the gateways used. The script that did it is not kept: its source system no longer exists, so it could only mislead a future reader into thinking it was runnable.
