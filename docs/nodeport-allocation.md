...
- **30881-30895**: Metrics endpoints for Prometheus
- New allocations should pick the next available port in the appropriate range. **Confirm against the live cluster, not just this table** — `kubectl get svc -A -o json | jq -r '.items[]|.spec.ports[]?|select(.nodePort)|.nodePort'`. This table has drifted before, and a collision surfaces as a Flux dry-run failure on the whole Kustomization, not as an obviously-wrong port.
